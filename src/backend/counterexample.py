...
class VcdVariable:
    ...
    def __init__(self, var_type: str, size: int, var_id: str, name: str):
...
class VcdScope:
    ...
    def find_var_by_id(self, var_id: str) -> VcdVariable:
        for var in self.variables:
            if var.id == var_id:
                return var
        raise ValueError("Variable not found in scope. Variable ID:" + var_id)

    def find_var_by_name(self, var_name: str) -> VcdVariable:
        for var in self.variables:
            if var.name == var_name:
                return var
        raise ValueError("Variable not found in scope. Variable name: " + var_name)

    def find_subscope_by_name(self, subscope_name: str):
        for scope in self.subscopes:
            if scope.name == subscope_name:
                return scope
        raise ValueError(f"Subscope {subscope_name} not found in scope {self.name}.")


# TODO: Combine CEX functionality in a class
var_bit_list = {}
...
def parse_var(file: StringIO, tokens: list[str]) -> tuple[list[str], VcdVariable]:
    ...
    var_bit_list[var.id] = var.size
    return tokens[6:], var


def parse_scope(file: StringIO, tokens: list[str]):
    """
...
def parse_value_changes(file: StringIO, tokens: list[str]) -> dict[int, dict[str, str]]:
    ...
    while True:
        ...
        # multi-bit variable change
            ...
            var_id = tokens[1]
            missing_bits = var_bit_list[var_id] - len(var_val)
            if missing_bits > 0:
                if var_val[0] == "1":
...
def parse_vcd_file(file: StringIO):
    ...
    :return: A tuple containing the hierarchy of scopes, signal value changes over time,
             and VCD metadata (date, version, and timescale)
    :rtype: (List[dict], Dict[int, Dict[str, str]], Tuple[str, str, str])
    ...
    while not structure_parsed:
        if not tokens:
            ...
            tokens, _ = parse_metadata(file, tokens)
    ...
    if len(scopes) != 1:
        raise ValueError("VCD format not supported, there must be exactly one top module.")
...
def identify_instances(scope: VcdScope) -> tuple[str, str]:
    ...
    # TODO: Instances should be identified by comparing the attributes of the scope objects
    # Top scope should have 3 instances: 2 identical instances + "checker_bind"
    if len(scope.subscopes) != 3:
        return "", ""
    instances = [subscope.name for subscope in scope.subscopes if "1" in subscope.name or "2" in subscope.name]
    if len(instances) != 2:
        return "", ""
...
def find_pairs(scope_a: VcdScope, scope_b: VcdScope, pairs: dict[str, str], var_names: dict[str, str], prefix="") -> None:
    ...
    for var_a in scope_a.variables:
        ...
        var_names[var_a.id] = prefix + "." + var_a.name if prefix else var_a.name
...
def analyze_cex(scope: VcdScope, value_changes: dict[int, dict[str, str]], instance_1_name: str, instance_2_name: str):
    ...
    deviations_by_time, first_deviations_by_time = find_value_deviations(value_changes, pairs, var_names)
